...
// Reset-isolated PMP lock and WARL test for VeeR-EH2.
//
// Checkpoints:
//      lock.cfg_warl
//      lock.pmpaddr0_warl
...
//      lock.load_deny_r0
//
// This binary deliberately creates locked PMP entries.  It must remain a
// standalone reset-isolated test because L=1 state persists until reset.
...
// Expected-fault support for lock.load_deny_r0.
...
// ---------------------------------------------------------------------------
// Locked-entry WARL behavior
//   cp0 = lock.cfg_warl
//   cp1 = lock.pmpaddr0_warl
//   cp2 = lock.pmpaddr1_warl
//   cp3 = lock.survives_clear
//   cp4 = lock.load_deny_r0, expected load-access fault cause 5
//
// Entry0 is locked NA4 R-only; entry1 is locked TOR R-only. The expected
// PMPCFG0 value is 0x8991:
//   byte0 = 0x91 = L=1, A=NA4, R=1
//   byte1 = 0x89 = L=1, A=TOR, R=1
// ---------------------------------------------------------------------------
t_lock_warl:
    dbg_char '8'
    clear_all_pmp

    // All addresses must be configured before any L=1 PMPCFG write.
    la   t3, guard_a_data
    srli t3, t3, 2
    csrw PMPADDR0, t3
    ...
    li   t1, (cfg_byte(1, A_TOR, 0, 0, 1) << 8)
    or   t0, t0, t1
    csrw PMPCFG0, t0
    pmp_sync

    // A write to a locked PMPCFG0 byte has no effect.
    set_cp 0
    csrw PMPCFG0, zero
    csrr t2, PMPCFG0
    li   t3, (cfg_byte(1, A_NA4, 0, 0, 1) | \
              (cfg_byte(1, A_TOR, 0, 0, 1) << 8))
    beq  t2, t3, 1f
    rec_failval t2, t3
    j    2f
1:  rec_pass
2:
    // PMPADDR0 is immutable because entry0 is locked. It is also the lower
    // bound used by locked TOR entry1.
    set_cp 1
    la   t3, guard_a_data
    srli t3, t3, 2
    li   t0, 0xFFFFF
...
3:  rec_pass
4:
    // PMPADDR1 is immutable because its own entry is locked.
    set_cp 2
    la   t3, guard_a_data
    addi t3, t3, 64
    srli t3, t3, 2
...
5:  rec_pass
6:
    // clear_all_pmp attempts to clear all PMP CSRs; the locked pair survives.
    set_cp 3
    clear_all_pmp
    csrr t2, PMPCFG0
    li   t3, (cfg_byte(1, A_NA4, 0, 0, 1) | \
...
7:  rec_pass
8:
    // Entry4 was cleared by clear_all_pmp because it was not previously
    // locked. Configure it as a separate locked 8-byte NAPOT R=0 region.
    // NAPOT 8-byte encoding uses the naturally aligned address >> 2.
    la   t0, guard_c_data
    srli t0, t0, 2
    csrw PMPADDR4, t0
    li   t0, cfg_byte(1, A_NAPOT, 0, 0, 0)
    csrw PMPCFG1, t0
    pmp_sync

    set_cp 4
    arm_trap 5
    la   t0, guard_c_data       // arm_trap clobbers t0
    lw   t1, 0(t0)              // R=0 and L=1 -> must fault
    check_trapped

    // Entry4 stays locked until reset; this binary ends after reporting.
    ret
...
trap_entry:
    ...
    // This trap was armed by the current checkpoint.
    sw   zero, 0(t3)
...
trap_wrong_cause:
    ...
    li   t5, ST_FAILCAUSE
    sw   t5, 0(t6)
    sw   t0, 4(t6)              // actual cause
    sw   t3, 8(t6)              // expected cause
    sw   t1, 12(t6)             // mtval
    addi t2, t2, 4
    csrw mepc, t2
    mret
...
s_h0_00: .asciz "lock.cfg_warl"
s_h0_01: .asciz "lock.pmpaddr0_warl"
s_h0_02: .asciz "lock.pmpaddr1_warl"
s_h0_03: .asciz "lock.survives_clear"
s_h0_04: .asciz "lock.load_deny_r0"

// Report strings
msg_banner:     .asciz "\nVeeR-EH2 PMP lock/WARL test (verbose)\n"
