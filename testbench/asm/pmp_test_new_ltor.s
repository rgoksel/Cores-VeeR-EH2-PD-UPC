...
// Reset-isolated PMP locked-TOR test for VeeR-EH2.
//
// Checkpoints:
//      ltor.load
//      ltor.store_fault
...
// It tests one locked TOR region from a clean reset.
...
// L=1 TOR R=1 W=0
//   cp0 = ltor.load
//   cp1 = ltor.store_fault, expected store-access fault cause 7
// ---------------------------------------------------------------------------
t_locked_tor_load_store:
    dbg_char '3'
    clear_all_pmp

    // Program the TOR bounds first, then lock entry1 with R=1/W=0.
    la   t0, guard_a_data
    srli t0, t0, 2
    csrw PMPADDR0, t0
    la   t0, guard_a_data
    ...
    li   t1, 0xdeadbeef
    sw   t1, 0(t0)              // W=0 and L=1 -> fault
    check_trapped

    // Entry1 remains locked until reset; this binary ends after reporting.
    clear_all_pmp
    ret
...
s_h0_00: .asciz "ltor.load"
s_h0_01: .asciz "ltor.store_fault"

msg_banner:     .asciz "\nVeeR-EH2 PMP locked-TOR test (verbose)\n"
